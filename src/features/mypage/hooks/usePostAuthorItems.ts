...
export const usePostAuthorItems = (authorId?: number) => {
  ...
  const getEquippedBorder = useUserProfileStore(
    (state) => state.getEquippedBorder,
  )
  const getEquippedNicknameColor = useUserProfileStore(
    (state) => state.getEquippedNicknameColor,
  )
