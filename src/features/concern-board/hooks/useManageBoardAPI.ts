...
export const useManageBoardAPI = () => {
  ...
  // 게시글 목록 조회
  const fetchBoardPosts = useCallback(
    async (page: number) => {
      setLoading(true)
      try {
  ...
  const searchBoardPosts = useCallback(
    async (title: string) => {
      setLoading(true)
      try {
        const token = localStorage.getItem('accessToken')
  ...
  // 신고된 게시글 목록 조회
  const fetchReportedBoards = useCallback(
    async (page: number) => {
      setReportsLoading(true)
      try {
        const token = localStorage.getItem('accessToken')
        const url = ENDPOINTS.REPORT_LIST_BY_TYPE(
          'BOARD',
          page,
          reportsPageSize,
        )
  ...
  // 신고된 댓글 목록 조회
  const fetchReportedComments = useCallback(
    async (page: number) => {
      setReportsLoading(true)
      try {
        const token = localStorage.getItem('accessToken')
  ...
  const updateBoard = useCallback(
    async (boardId: number, updateData: unknown) => {
      const token = localStorage.getItem('accessToken')
